...
function serializeProof(proof) {
    switch (proof.type) {
        case ProofType.single: {
            const output = new Uint8Array(1 + single_1.computeSingleProofSerializedLength(proof.witnesses));
            output[0] = exports.ProofTypeSerialized.indexOf(ProofType.single);
            single_1.serializeSingleProof(output, 1, proof.gindex, proof.leaf, proof.witnesses);
            return output;
        }
        case ProofType.multi:
...
function deserializeProof(data) {
    ...
    switch (proofType) {
        case ProofType.single: {
            const [gindex, leaf, witnesses] = single_1.deserializeSingleProof(data, 1);
            return {
                type: ProofType.single,
                gindex,
                leaf,
