...
async function main() {
  ...
  if (process.argv[2]) {
    if (!process.argv[3]) {
      ...
      const pangolinEndpoint = PangolinEndpoint.attach(
        await caller.endpointAddress()
      );
      console.log(`pangolinEndpoint: ${await caller.endpointAddress()}`);
      console.log(
        `pangoroEndpoint: ${await pangolinEndpoint.remoteEndpoint()}`
      );

      const fee = await pangolinEndpoint.fee();
      console.log(`market fee: ${fee}`);
      const tx = await caller.remoteAdd(calleeAddress, {
        value: fee,
      });
