...
contract LightClientBridge is Bitfield, ValidatorRegistry, GuardRegistry {
    ...
    */
     ...
     * @param commitBlockNumber the block number of commitmentHash produced
    ...
    */
     ...
     * @dev `commitmentHash` must be matched to the `commitBlockNumber`, if not the collateral may be slashed.
    ...
    /**
     * @notice Relyer could claim the expired commitment's collateral,w
    ...
    function verifyCommitment(
        ...
        /**
         * @dev verify that blockNumber is the same as in `newSignatureCommitment`
         */
        require(
            commitment.blockNumber == data.commitBlockNumber,
            "Bridge: blockNumber does not match original validation data"
        );
        ...
        /**
         * @notice If relayer submit correct commitmentHash in `newSignatureCommitment`, payback the collateral.
         * else slash the collateral.
         */
        if (commitmentHash == data.commitmentHash) {
            data.senderAddress.transfer(MIN_SUPPORT);
        } else {
