...
BSCLightClient=$(jq -r ".$NETWORK_NAME.BSCLightClient" "$PWD/bin/addr/$NETWORK.json")
seth send -F $ETH_FROM $BSCLightClient "registry(uint32,uint32,address,uint32,address)" $bridged_chain_pos $this_out_lane_pos $OutboundLane $this_in_lane_pos $InboundLane --rpc-url http://127.0.0.1:9545
