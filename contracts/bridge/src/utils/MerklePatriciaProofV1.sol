...
library MerklePatriciaProofV1 {
    ...
    function isEmptyBytesequence(RLPDecode.RLPItem memory item)
        internal
        pure
        returns (bool)
    {
    ...
    function validateMPTProof(
        ...
        RLPDecode.RLPItem[] memory stack = proof.toRlpItem().toList();
        mptKey = decodeNibbles(mptKey, 0, 1);
        uint256 mptKeyOffset = 0;

        bytes32 nodeHashHash;
        bytes memory rlpNode;
        RLPDecode.RLPItem[] memory node;

        RLPDecode.RLPItem memory rlpValue;
